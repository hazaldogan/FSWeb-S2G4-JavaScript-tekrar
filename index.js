...
ucebolunenlerintoplami = ucetambolunenler.reduce(
  (ucebolunenlerintoplami, sayi) => ucebolunenlerintoplami + sayi
);
...
for (let element of sayilar) {
  if (tekrarSayi[element] !== undefined) {
    tekrarSayi[element]++;
  } else {
    tekrarSayi[element] = 1;
  }
}
for (let key in tekrarSayi) {
  if (tekrarSayi[key] !== 1) {
    tekraredensayilar.push(
      `${key} sayısı ${tekrarSayi[key]} kere tekrar edilmiştir`
    );
  }
}
